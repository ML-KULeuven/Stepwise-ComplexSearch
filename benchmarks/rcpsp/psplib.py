...
class PSPLibDataset(object):  # torch.utils.data.Dataset compatible
    ...
    def __getitem__(self, index: int|str) -> Tuple[Any, Any]:
        ...
        files = natsorted(list(self.family_dir.glob(f"*.{self.family_codes[self.variant]}")))
...
def parse_rcpsp(filename: str) -> dict:
    ...
    with open(filename, "r") as f:
        ...
        while not line.startswith("*****"):
            ...
            assert len(resources) == len(resource_names), "Expected %d resources for job %d, got %d" % (
                len(resource_names), jobnr, len(resources))
...
def rcpsp_model(job_data, capacities):
    ...
    for rescource, capa in capacities.items():
        model += cp.Cumulative(
            ...
            end=None, # End variables not required anymore
...
if __name__ == "__main__":
    ...
    # assert model.solve(solver="pumpkin")
    #
    ...
    for data, metadata in dataset:
        if metadata['name'].endswith("_1"):
            instances.append(metadata['name'])

    print(str(instances).replace("'",'"'))
